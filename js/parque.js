...
function init(){
  ...
  TALLERES.forEach(t=>{
    ...
    m.bindPopup(popup);
    // rollover en escritorio, click/tap en todos (móvil)
    m.on('mouseover',function(){if(!IS_TOUCH)this.openPopup();});
...
function doUpdate(){
  ...
  entries.forEach(([mi,n])=>{
    ...
    const r=Math.max(13,10+24*Math.sqrt(n/maxM));
    ...
    mk.on('mouseover',function(){if(!IS_TOUCH)this.openPopup();});
