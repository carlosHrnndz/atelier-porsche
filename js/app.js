...
function applyCatalogPreset(presetName) {
  ...
  for (const catKey in catalog) {
    catalog[catKey].items.forEach((item, idx) => {
      ...
      } else if (presetName === 'estudios' || presetName === 'unsocio') {
      ...
      } else if (presetName === 'acabar') {
        const opts = item.opciones;
        const lastOpt = opts[opts.length - 1];
        S[itemKey + '_opt'] = lastOpt ? lastOpt.letra : 'A';
        S[itemKey + '_qty'] = 1;
      }
...
$$('.preset').forEach(p=>p.addEventListener('click',()=>{
  ...
  setNs(S.ns);setSocioProd(S.socioProd);syncAll();
...
function renderAdvancedConfigurator() {
  ...
  activeCat.items.forEach((item, idx) => {
    ...
    item.opciones.forEach(opt => {
      ...
      const sourceLinkHtml = opt.fuente && opt.fuente.startsWith('http') ? `
        <a href="${opt.fuente}" target="_blank" class="cx-opt-link">🔗 Ver ficha del producto &rarr;</a>
      ` : '';
...
async function initApp() {
  try {
    const res = await fetch('./data/equipment_catalog.json?v=16');
