...
function syncQtyAndPriceFromTotals() {
  const QTY_ITEMS = ['cx_eleva', 'cx_aircon', 'cx_bie', 'cx_domkit', 'cx_carga', 'cx_prensa', 'cx_grua', 'cx_alineadora'];
  QTY_ITEMS.forEach(f => {
    if (S[f] === 0) {
      S[f+'_qty'] = 0;
    } else {
...
function exportMarkdown(){
  ...
  md+=`## Tesorería mes a mes\n\n| Mes | Horas fact. | Facturación | Margen % | OPEX | Resultado | Caja fin |\n|---|---|---|---|---|---|---|\n`;
  R.months.forEach(m=>{md+=`| ${m.m} | ${m.h} | ${eur(m.facturacion)} | ${m.facturacion>0?(m.margenPct*100).toFixed(1)+'%':'—'} | ${eur(m.opex)} | ${eur(m.res)} | ${eur(m.fin)} |\n`;});
...
function renderPnl(R){
  ...
  body+=row('Margen taller (MO+recambios)',m=>m.mc-m.domIncome-(m.cpIncome||0));
  body+=row('Diagnóstico a domicilio',m=>m.domIncome);
  if(S.cpEnable) body+=row('Chapa y pintura',m=>m.cpIncome||0);
  if(S.f2Enable) body+=row('Compraventa (neto caja)',m=>m.f2In-m.f2Out);
  body+=row('FACTURACIÓN TOTAL',m=>m.facturacion,'sec-label');
  ...
  body+='<tr class="pnl-margin"><td>% margen s/ facturación</td>'+ms.map(m=>{const v=m.margenPct*100;return '<td style="color:'+(v<0?'var(--red-soft)':'var(--green-soft)')+'">'+(m.facturacion>0?v.toFixed(1)+'%':'—')+'</td>';}).join('')+'</tr>';
