...
function drawChart(R){
  ...
  if(chartLayers.facturacion){
    ...
    ptsFact.forEach(([x,y])=>{
      svg+=`<circle class="chart-layer-facturacion" cx="${x}" cy="${y}" r="3" fill="var(--gold)" opacity=".85" pointer-events="none"></circle>`;
    });
  }
  
  // 4. Interactive hover layer
  const showEvery=N>14?Math.ceil(N/12):1;
  R.months.forEach((d,i)=>{
    const x=X(i);
    // Find target Y position based on priorities of active layers (Caja > Facturacion > Beneficio)
    let targetYVal = 0;
    ...
    const info=`Mes ${d.m}  ·  Caja: ${eur(d.fin)}  ·  Bº Neto: ${eur(d.netProfit)}  ·  Bº Bruto: ${eur(d.beneficioBruto)}  ·  Facturación: ${eur(d.facturacion)}  ·  ${d.h} h facturables`;
    ...
    if (chartLayers.caja || chartLayers.facturacion || chartLayers.beneficio) {
