...
function compute(){
  ...
  for(let i=0;i<N;i++){
    ...
    let facturableHours = totalProducers * S.presence * prod;
    // domicilio: consumes some facturable hours but brings fee income + funnels work
    const domVisits = dom ? S.domVisitsM * Math.min(1, 0.4+0.6*t) : 0; // ramps with the rest
    ...
    const domCost = dom ? (S.domVehLease+S.domFuel+S.domSeg) : 0;
    let opex=alq+salaries+S.piwis+S.sumi+S.seg+S.ges+S.mkt+S.limpieza+S.lavadoMonos+domCost+leaseC+icoPay;
    ...
    const res=mc-opex+ayudaPuntual; const ini=caja; caja=ini+res;
    if(caja<0&&insolv===null) insolv=i+1;
    if(res>=0&&be===null) be=i+1;
    if(res>best) best=res;
    months.push({m:i+1,h:Math.round(facturableHours),prod,producers:totalProducers,mo,mc,opex,res,ini,fin:caja,salaries,alq,leaseC,icoPay,limpieza:S.limpieza,lavadoMonos:S.lavadoMonos,domCost,domIncome,domVisits:Math.round(domVisits),ayudaPuntual});
  }
...
function applyState(o){
  ...
  setNs(S.ns);setSocioProd(S.socioProd);setDomEnable(S.domEnable);syncAll();render();
}
