...
function syncQtyAndPriceFromTotals() {
  const SIMPLE_QTY_ITEMS = ['cx_eleva', 'cx_aircon', 'cx_bie', 'cx_domkit', 'cx_carga', 'cx_prensa', 'cx_grua', 'cx_alineadora', 'cx_pintura', 'cx_obra', 'cx_it', 'cx_software', 'cx_stock', 'cx_fianza', 'cx_neumatica', 'cx_extrac', 'cx_residuos', 'cx_epis', 'cx_mobtaller', 'cx_pci', 'cx_intrusion'];
  
  const defaultPrices = {
    cx_eleva: 8000, cx_aircon: 5000, cx_bie: 600, cx_domkit: 6000, cx_carga: 1500,
    cx_prensa: 1200, cx_grua: 900, cx_alineadora: 15000, cx_pintura: 22000,
    cx_obra: 30000, cx_it: 1500, cx_software: 1000, cx_stock: 6000, cx_fianza: 5300,
    cx_neumatica: 5500, cx_extrac: 2500, cx_residuos: 1800, cx_epis: 1500,
    cx_mobtaller: 3000, cx_pci: 4000, cx_intrusion: 2200
  };
  
  SIMPLE_QTY_ITEMS.forEach(f => {
    if (S[f] === 0) {
      S[f+'_qty'] = 0;
    } else {
      const price = S[f+'_price'] || defaultPrices[f];
  ...
  const subitemKeys = [
    'cx_herram_carros', 'cx_herram_bancos', 'cx_herram_compresor', 'cx_herram_dinamo', 'cx_herram_mano', 'cx_herram_neumat',
    'cx_diag_piwis', 'cx_diag_utiles',
    'cx_aux_gatos', 'cx_aux_lavadora', 'cx_aux_aceite',
    'cx_mob_boutique', 'cx_mob_oficina', 'cx_mob_estanterias',
    'cx_rotulo_lum', 'cx_rotulo_int',
    'cx_licencia_proj', 'cx_licencia_tasas'
  ];
  
  const subitemDefaults = {
    cx_herram_carros: 1500, cx_herram_bancos: 1000, cx_herram_compresor: 3500, cx_herram_dinamo: 400, cx_herram_mano: 800, cx_herram_neumat: 700,
    cx_diag_piwis: 4500, cx_diag_utiles: 2500,
    cx_aux_gatos: 800, cx_aux_lavadora: 1200, cx_aux_aceite: 600,
    cx_mob_boutique: 3000, cx_mob_oficina: 1000, cx_mob_estanterias: 1000,
    cx_rotulo_lum: 2500, cx_rotulo_int: 1500,
    cx_licencia_proj: 6000, cx_licencia_tasas: 4000
  };
  
  const parentOfSub = {
    cx_herram_carros: 'cx_herram', cx_herram_bancos: 'cx_herram', cx_herram_compresor: 'cx_herram', cx_herram_dinamo: 'cx_herram', cx_herram_mano: 'cx_herram', cx_herram_neumat: 'cx_herram',
    cx_diag_piwis: 'cx_diag', cx_diag_utiles: 'cx_diag',
    cx_aux_gatos: 'cx_aux', cx_aux_lavadora: 'cx_aux', cx_aux_aceite: 'cx_aux',
    cx_mob_boutique: 'cx_mob', cx_mob_oficina: 'cx_mob', cx_mob_estanterias: 'cx_mob',
    cx_rotulo_lum: 'cx_rotulo', cx_rotulo_int: 'cx_rotulo',
    cx_licencia_proj: 'cx_licencia', cx_licencia_tasas: 'cx_licencia'
  };
  
  const parentSums = {};
  subitemKeys.forEach(k => {
    const parent = parentOfSub[k];
    if (S[parent] === 0) {
      S[k+'_qty'] = 0;
    } else {
      if (S[k+'_qty'] === undefined) S[k+'_qty'] = k.includes('carros') || k.includes('bancos') || k.includes('gatos') || k.includes('estanterias') || k.includes('oficina') || k.includes('neumat') ? 2 : (k.includes('dinamo') || k.includes('mano') ? 3 : 1);
      if (S[k+'_price'] === undefined) S[k+'_price'] = subitemDefaults[k];
    }
    S[k] = S[k+'_qty'] * S[k+'_price'];
    parentSums[parent] = (parentSums[parent] || 0) + S[k];
  });
  
  for(const p in parentSums){
    if (S[p] !== 0) S[p] = parentSums[p];
  }
}
...
const CAPEX_SUBITEMS = {
  cx_herram: [
    { k: 'cx_herram_carros', label: 'Carros herramientas', help: 'Carros completos de herramientas (Beta/Facom)' },
    { k: 'cx_herram_bancos', label: 'Bancos de trabajo', help: 'Bancos de taller con paneles y estantes' },
    { k: 'cx_herram_compresor', label: 'Compresor de tornillo', help: 'Compresor industrial de tornillo' },
    { k: 'cx_herram_dinamo', label: 'Llaves dinamométricas', help: 'Calibradas para motor y chasis' },
    { k: 'cx_herram_mano', label: 'Herramienta manual', help: 'Juegos de fijas, vasos, alicates, destornilladores' },
    { k: 'cx_herram_neumat', label: 'Pistolas de impacto', help: 'Pistolas de impacto y útiles neumáticos' }
  ],
  cx_diag: [
    { k: 'cx_diag_piwis', label: 'Diagnosis PIWIS (HW)', help: 'Hardware oficial de diagnosis Porsche' },
    { k: 'cx_diag_utiles', label: 'Útiles específicos', help: 'Extractores y útiles específicos de calado de motor' }
  ],
  cx_aux: [
    { k: 'cx_aux_gatos', label: 'Gatos de carretilla/foso', help: 'Gatos hidráulicos profesionales' },
    { k: 'cx_aux_lavadora', label: 'Limpiador de piezas', help: 'Lavadora de piezas recirculante' },
    { k: 'cx_aux_aceite', label: 'Recuperador de aceite', help: 'Aspirador/recuperador de fluidos' }
  ],
  cx_mob: [
    { k: 'cx_mob_boutique', label: 'Sofás/mesas boutique', help: 'Mobiliario zona de espera boutique' },
    { k: 'cx_mob_oficina', label: 'Mesas/sillas oficina', help: 'Escritorios para recepción y administración' },
    { k: 'cx_mob_estanterias', label: 'Estanterías boutique', help: 'Expositores de recambios y boutique' }
  ],
  cx_rotulo: [
    { k: 'cx_rotulo_lum', label: 'Rótulo luminoso', help: 'Identificación corporativa en fachada' },
    { k: 'cx_rotulo_int', label: 'Señalética interior', help: 'Placas interiores e imagen boutique' }
  ],
  cx_licencia: [
    { k: 'cx_licencia_proj', label: 'Proyecto ingeniería', help: 'Memoria y dirección de obra de ingeniero' },
    { k: 'cx_licencia_tasas', label: 'Tasas municipales', help: 'Tasas de actividad e inspección municipal' }
  ]
};
...
if(capexHost){
  CAPEX_ITEMS.forEach(([f,label,help])=>{
    ...
    if(CAPEX_SUBITEMS[f]){
      const subs = CAPEX_SUBITEMS[f];
      d.className = 'acc-sub-container';
      let html = `<details class="acc-sub"><summary><span>${label} <span class="help" data-help="${help.replace(/"/g,'&quot;')}">i</span></span><b id="cxv_${f}" class="cx-total-val"></b></summary>
        <div class="acc-sub-content">`;
      
      subs.forEach(sub=>{
        html += `<div class="field sub-field">
          <div class="lab"><span>${sub.label} <span class="help" data-help="${sub.help.replace(/"/g,'&quot;')}">i</span></span><b id="cxv_${sub.k}" class="cx-sub-val"></b></div>
          <div class="cx-qty-row">
            <div class="cx-qty-col">
              <span class="cx-qty-label">Uds:</span>
              <input type="number" class="num cx-qty-input" id="cxqty_${sub.k}" min="0" max="20" step="1" value="${S[sub.k+'_qty']}">
            </div>
            <div class="cx-qty-col">
              <span class="cx-qty-label">Unit (€):</span>
              <input type="number" class="num cx-price-input" id="cxprc_${sub.k}" min="0" max="100000" step="100" value="${S[sub.k+'_price']}">
            </div>
          </div>
        </div>`;
      });
      ...
      subs.forEach(sub=>{
        const qtyInp = d.querySelector('#cxqty_' + sub.k);
        const prcInp = d.querySelector('#cxprc_' + sub.k);
        const updateSubVal = () => {
          S[sub.k+'_qty'] = Math.max(0, parseInt(qtyInp.value) || 0);
          S[sub.k+'_price'] = Math.max(0, parseInt(prcInp.value) || 0);
          const subTotal = S[sub.k+'_qty'] * S[sub.k+'_price'];
          d.querySelector('#cxv_' + sub.k).textContent = eur(subTotal);
          
          S[f] = subs.reduce((accSum, curr) => accSum + S[curr.k+'_qty'] * S[curr.k+'_price'], 0);
          d.querySelector('#cxv_' + f).textContent = eur(S[f]);
          
          clearActive();
          render();
        };
        qtyInp.addEventListener('input', updateSubVal);
        prcInp.addEventListener('input', updateSubVal);
        fields[sub.k] = { qtyInp, prcInp, isCapex: true, isQty: true };
      });
    } else {
      d.className='field';
...
function syncAll(){
  ...
  for (const parent in CAPEX_SUBITEMS) {
    const el = $('#cxv_' + parent);
    if (el) el.textContent = eur(S[parent]);
    CAPEX_SUBITEMS[parent].forEach(sub => {
      const subEl = $('#cxv_' + sub.k);
      if (subEl) subEl.textContent = eur(S[sub.k]);
    });
  }
}
...
function compute(){
  // Recalculate parent categories from sub-items
  S.cx_herram_carros = S.cx_herram_carros_qty * S.cx_herram_carros_price;
  S.cx_herram_bancos = S.cx_herram_bancos_qty * S.cx_herram_bancos_price;
  S.cx_herram_compresor = S.cx_herram_compresor_qty * S.cx_herram_compresor_price;
  S.cx_herram_dinamo = S.cx_herram_dinamo_qty * S.cx_herram_dinamo_price;
  S.cx_herram_mano = S.cx_herram_mano_qty * S.cx_herram_mano_price;
  S.cx_herram_neumat = S.cx_herram_neumat_qty * S.cx_herram_neumat_price;
  S.cx_herram = S.cx_herram_carros + S.cx_herram_bancos + S.cx_herram_compresor + S.cx_herram_dinamo + S.cx_herram_mano + S.cx_herram_neumat;
  
  S.cx_diag_piwis = S.cx_diag_piwis_qty * S.cx_diag_piwis_price;
  S.cx_diag_utiles = S.cx_diag_utiles_qty * S.cx_diag_utiles_price;
  S.cx_diag = S.cx_diag_piwis + S.cx_diag_utiles;
  
  S.cx_aux_gatos = S.cx_aux_gatos_qty * S.cx_aux_gatos_price;
  S.cx_aux_lavadora = S.cx_aux_lavadora_qty * S.cx_aux_lavadora_price;
  S.cx_aux_aceite = S.cx_aux_aceite_qty * S.cx_aux_aceite_price;
  S.cx_aux = S.cx_aux_gatos + S.cx_aux_lavadora + S.cx_aux_aceite;
  
  S.cx_mob_boutique = S.cx_mob_boutique_qty * S.cx_mob_boutique_price;
  S.cx_mob_oficina = S.cx_mob_oficina_qty * S.cx_mob_oficina_price;
  S.cx_mob_estanterias = S.cx_mob_estanterias_qty * S.cx_mob_estanterias_price;
  S.cx_mob = S.cx_mob_boutique + S.cx_mob_oficina + S.cx_mob_estanterias;
  
  S.cx_rotulo_lum = S.cx_rotulo_lum_qty * S.cx_rotulo_lum_price;
  S.cx_rotulo_int = S.cx_rotulo_int_qty * S.cx_rotulo_int_price;
  S.cx_rotulo = S.cx_rotulo_lum + S.cx_rotulo_int;
  
  S.cx_licencia_proj = S.cx_licencia_proj_qty * S.cx_licencia_proj_price;
  S.cx_licencia_tasas = S.cx_licencia_tasas_qty * S.cx_licencia_tasas_price;
  S.cx_licencia = S.cx_licencia_proj + S.cx_licencia_tasas;

  // Simple items
  const ALL_QTY_ITEMS = ['cx_eleva', 'cx_aircon', 'cx_bie', 'cx_domkit', 'cx_carga', 'cx_prensa', 'cx_grua', 'cx_alineadora', 'cx_pintura', 'cx_obra', 'cx_it', 'cx_software', 'cx_stock', 'cx_fianza', 'cx_neumatica', 'cx_extrac', 'cx_residuos', 'cx_epis', 'cx_mobtaller', 'cx_pci', 'cx_intrusion'];
  ALL_QTY_ITEMS.forEach(f => {
    S[f] = S[f+'_qty'] * S[f+'_price'];
  });
  const N=S.horizon;
...
function drawChart(R){
  ...
  if(chartLayers.caja){
    ...
    pts.forEach(([x,y], i)=>{
      ...
      svg+=`<circle class="chart-layer-caja" cx="${x}" cy="${y}" r="3.2" fill="${dotColor}" stroke="var(--bg)" stroke-width="1.5" pointer-events="none"></circle>`;
    });
  }
  
  // 3. Facturación (Line & Markers)
  if(chartLayers.facturacion){
    ...
    ptsFact.forEach(([x,y])=>{
      svg+=`<circle class="chart-layer-facturacion" cx="${x}" cy="${y}" r="3.2" fill="var(--gold-soft)" stroke="var(--bg)" stroke-width="1.5" pointer-events="none"></circle>`;
    });
  }

  // 4. Gastos (Line & Markers)
  if(chartLayers.gastos){
    ...
    ptsGast.forEach(([x,y])=>{
      svg+=`<circle class="chart-layer-gastos" cx="${x}" cy="${y}" r="3.2" fill="var(--purple-soft)" stroke="var(--bg)" stroke-width="1.5" pointer-events="none"></circle>`;
    });
  }
  
  // 5. Interactive hover layer
  const showEvery=N>14?Math.ceil(N/12):1;
  R.months.forEach((d,i)=>{
    const x=X(i);
    // Find target Y position based on priorities of active layers (Caja > Facturacion > Gastos > Beneficio)
    let targetYVal = 0;
    if (chartLayers.caja) targetYVal = d.fin;
    else if (chartLayers.facturacion) targetYVal = d.facturacion;
    else if (chartLayers.gastos) targetYVal = d.opex;
    else if (chartLayers.beneficio) targetYVal = d.netProfit;
    const y = Y(targetYVal);
    
    const info=`Mes ${d.m}  ·  Caja: ${eur(d.fin)}  ·  Gastos: ${eur(d.opex)}  ·  Bº Neto: ${eur(d.netProfit)}  ·  Bº Bruto: ${eur(d.beneficioBruto)}  ·  Facturación: ${eur(d.facturacion)}  ·  ${d.h} h facturables`;
    ...
    if (chartLayers.caja || chartLayers.facturacion || chartLayers.beneficio || chartLayers.gastos) {
      let dotColor = 'var(--gold-soft)';
      if (chartLayers.caja) dotColor = d.fin >= 0 ? 'var(--green-soft)' : 'var(--red-soft)';
      else if (chartLayers.facturacion) dotColor = 'var(--gold-soft)';
      else if (chartLayers.gastos) dotColor = 'var(--purple-soft)';
      else if (chartLayers.beneficio) dotColor = d.netProfit >= 0 ? 'var(--green-soft)' : 'var(--red-soft)';
      svg+=`<circle class="pt-dot" cx="${x}" cy="${y}" r="3" fill="${dotColor}" stroke="var(--bg)" stroke-width="1.5" pointer-events="none"></circle>`;
    }
