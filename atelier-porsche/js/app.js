...
const S={
  ...
  cx_domkit:6000, cx_neumatica:5500, cx_aircon:5000, cx_alineadora:0,
  ...
  domEnable:1, domVehLease:420, domFuel:250, domSeg:90, domVisitsM:20, domFee:60, domHours:0.75,
  ...
  estacional:1, mesApertura:9, impuestos:1, garantiaPct:0.02, ipc:0.03,
  // Fase 2 · compraventa
...
function compute(){
  ...
  let caja=S.cap+S.ico-capexT+ayudaCapexInicial, insolv=null, be=null, best=-1e12;
  ...
  for(let i=0;i<N;i++){
    ...
    const socioMechs = S.socioProd ? (S.ns * S.socioProdPct) : 0;
    ...
    // domicilio: consumes some facturable hours but brings fee income + funnels work
    const domVisits = dom ? S.domVisitsM * Math.min(1, 0.4+0.6*t) * season : 0;
    ...
    if(S.f2Enable){
      if(i+1>=S.f2MesInicio){
        f2Out = S.f2CochesMes * S.f2Capital;                       // compras de este mes
        // horas de reacondicionado: primero salen de capacidad ociosa, solo restan facturación si no hay
      ...
      if(i+1-S.f2CicloMeses>=S.f2MesInicio){                       // ventas: coches comprados hace 'ciclo' meses
    ...
    const mc=mo + mo*S.ratio*S.mgn + domIncome;
    // provisión de garantía: % de la facturación (MO + recambios facturados)
    const garProv = S.garantiaPct * (mo + mo*S.ratio);
    ...
    const domCost = dom ? (S.domVehLease+S.domFuel+S.domSeg)*ipcF : 0;
    let opex=(alq+S.piwis+S.sumi+S.seg+S.ges+S.mkt+S.limpieza+S.lavadoMonos+S.pciMant+S.cra)*ipcF + salaries + domCost + leaseC + icoPay;
    // Tarifa Cero: solo aplica durante los primeros 12 meses
    if(i<12) opex -= ayudaOpexMes;
    // subvención por contratación indefinida: ingreso puntual el mes en que se contrata
    let ayudaPuntual=0;
    if(i===0 && ayudaContrata>0) ayudaPuntual += ayudaContrata; // por el jefe (mes 1)
    if(i+1===S.mecMonth && S.mecCount>0 && S[ 'ay_contrata' ]) ayudaPuntual += 6000*S.mecCount; // mecánicos extra
    // impuesto de sociedades: liquidación al cierre de cada año con bases negativas
    ...
    if(S.impuestos && (i+1)%12===0){
      if(annAcc>0){
        ...
        const rate=(Math.floor(i/12)<2)?0.15:0.25; // tipo reducido nuevas empresas 2 primeros años
        taxPay=taxable*rate;
      } else lossCarry += -annAcc;
      annAcc=0;
    }
    const res=mc-opex+ayudaPuntual-garProv-f2Out+f2In-taxPay; const ini=caja; caja=ini+res;
    if(caja<0&&insolv===null) insolv=i+1;
    if(res>=0&&be===null) be=i+1;
    if(res>best) best=res;
    months.push({m:i+1,h:Math.round(facturableHours),prod,producers:totalProducers,mo,mc,opex,res,ini,fin:caja,salaries,alq,leaseC,icoPay,limpieza:S.limpieza,lavadoMonos:S.lavadoMonos,domCost,domIncome,domVisits:Math.round(domVisits),ayudaPuntual,season,garProv,f2Out,f2In,taxPay});
  }
  const last=months[N-1];
  const beH=last.opex/(S.tar*(1+S.ratio*S.mgn));
  return {alq,leaseC,capexT,months,insolv,be,best,cajaFin:caja,beH,cajaIni:S.cap+S.ico-capexT+ayudaCapexInicial,ayudaCapexInicial,ayudaOpexMes};
}
...
function exportMarkdown(){
  ...
  md+=`## Tesorería mes a mes\n\n| Mes | Horas fact. | Margen | OPEX | Resultado | Caja fin |\n|---|---|---|---|---|---|\n`;
  R.months.forEach(m=>{md+=`| ${m.m} | ${m.h} | ${eur(m.mc)} | ${eur(m.opex)} | ${eur(m.res)} | ${eur(m.fin)} |\n`;});
...
function renderPnl(R){
  const tbl=$('#pnlTable');if(!tbl)return;
  const ms=R.months;
  const head='<thead><tr><th>Concepto</th>'+ms.map(m=>'<th>M'+m.m+'</th>').join('')+'</tr></thead>';
  const row=(label,fn,cls)=>'<tr'+(cls?' class="'+cls+'"':'')+'><td>'+label+'</td>'+ms.map(m=>{const v=fn(m);return '<td style="color:'+(v<0?'var(--red-soft)':'inherit')+'">'+(v===null?'—':eur(v))+'</td>';}).join('')+'</tr>';
  let body='<tbody>';
  body+='<tr class="sec-label"><td>Ingresos</td>'+ms.map(()=>'<td></td>').join('')+'</tr>';
  body+=row('Margen taller (MO+recambios)',m=>m.mc-m.domIncome);
  body+=row('Diagnóstico a domicilio',m=>m.domIncome);
  if(S.f2Enable) body+=row('Compraventa (neto caja)',m=>m.f2In-m.f2Out);
  body+='<tr class="sec-label"><td>Costes</td>'+ms.map(()=>'<td></td>').join('')+'</tr>';
  body+=row('OPEX total',m=>-m.opex);
  body+=row('Provisión garantía',m=>-m.garProv);
  body+=row('Impuestos (IS)',m=>m.taxPay?-m.taxPay:null);
  body+='<tr class="sec-label"><td>Resultado</td>'+ms.map(()=>'<td></td>').join('')+'</tr>';
  body+=row('Resultado del mes',m=>m.res);
  body+=row('CAJA FIN DE MES',m=>m.fin);
  body+='</tbody>';
